  ...
  */
  ...
  protected function checkTestCaseVersionNumber()
  {
   ...
   /*
    ...
    returns: 
   */
   public function getSubTree($args)
   {
        $msg_prefix="(" .__FUNCTION__ . ") - ";
        $status_ok=true;
        $this->_setArgs($args);
        ...
        if( $status_ok )
        {
            $result = $this->tprojectMgr->get_subtree($this->args[self::$testSuiteIDParamName], true);
            if( is_null($result) )
            {
                $status_ok=false;
                $msg = 'getSubTree failed: test suite may be empty.';
